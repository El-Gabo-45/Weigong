...
export function isSquareProtectedByArcher(state, targetRow, targetCol, protectorSide) {
    ...
    for (let r = 0; r < BOARD_SIZE; r++) {
        ...
        for (let c = 0; c < BOARD_SIZE; c++) {
            const p = board[r][c];
            if (!p || p.type !== "archer" || p.side !== protectorSide)
                continue;
...
export function getArcherAmbushResult(state, archer, bankPos) {
    ...
    if (cannotRetreat.length === victims.length)
        return { type: 'autoCaptureAll', victims: victims.map(v => ({ r: v.r, c: v.c })), victimPieces: victims.map(v => ({ ...v.piece })), archerTo: bankPos };
    if (victims.length === 1)
        return { type: 'singleCapture', victim: { r: victims[0].r, c: victims[0].c }, victimPiece: { ...victims[0].piece }, archerTo: bankPos };
    ...
    if (victims.length === 1) {
        ...
        for (const v of victims) {
            if (v === chosen)
                continue;
            if (v.canRetreat) {
                state.board[v.r + front][v.c] = v.piece;
                state.board[v.r][v.c] = null;
            }
            else {
                captureToReserve(state, v.piece, archer.side);
                state.board[v.r][v.c] = null;
            }
        }
    }
