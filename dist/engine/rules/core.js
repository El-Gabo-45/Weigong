...
// ═════════════════════════════════════════════════════
import { BOARD_SIZE, SIDE, opponent, isPalaceSquare, onBank, forwardDir, homePromotionZone, canDropOnSide, isPromotableType, pieceLabel, isRiverSquare } from "../constants.js";
import { makePiece, findKings, boardSignature, cloneState } from './board.js';
import { isSquareProtectedByArcher, getArcherAmbushResult } from './archer.js';
import { isKingInCheck } from './check.js';
import { getLegalMovesForSquare } from './moves.js';
import { updatePalaceState } from './state.js';
import { computeFullHash } from '../ai/hashing.js';
import { captureToReserve } from './capture.js';
function maybePromote(state, piece, r, c) {
    if (!piece || !isPromotableType(piece.type) || piece.promoted)
        return false;
    if (!homePromotionZone(piece.side, r))
        return false;
    return true;
}
...
function moveSignature(action, side) {
    ...
    if (action.fromReserve)
        return `drop:${side}:${action.reserveIndex}->${action.to.r},${action.to.c}:${action.promotion ? 1 : 0}`;
...
export function applyMove(state, action) {
    ...
    if (state.lastRepeatedMoveKey != null && currentMoveKey != null && state.lastRepeatedMoveKey === currentMoveKey) {
    ...
    if (fromReserve) {
        ...
        if (target && target.side !== movingPiece.side)
            captureToReserve(state, target, movingPiece.side);
        board[to.r][to.c] = movingPiece;
        board[from.r][from.c] = null;
        state.lastMove = { from, to, piece: movingPiece.type };
    }
    if (maybePromote(state, movingPiece, to.r, to.c) && promotion)
        applyPromotion(movingPiece);
    ...
    if (!silent && movingPiece.type === "archer" && onBank(movingPiece.side, to.r) && !fromReserve) {
        ...
        if (ambushResult)
            state.archerAmbush = ambushResult;
    }
    if (!silent) {
...
function isInPalaceMate(state, side) {
    ...
    if (!king || !isPalaceSquare(king.r, king.c, side))
        return false;
    const enemy = opponent(side);
    let enemyInside = false;
    for (let r = 0; r < BOARD_SIZE && !enemyInside; r++) {
        for (let c = 0; c < BOARD_SIZE; c++) {
            if (state.board[r][c]?.side === enemy && isPalaceSquare(r, c, side)) {
                enemyInside = true;
                break;
            }
        }
    ...
    if (kingMoves.some(m => !isPalaceSquare(m.r, m.c, side)))
        return false;
...
export function getAllLegalMoves(state, side) {
    ...
    for (const drop of getLegalReserveDrops(state, side))
        all.push(drop);
    return all;
}
...
export function getLegalReserveDrops(state, side) {
    ...
    for (let r = 0; r < BOARD_SIZE; r++) {
        for (let c = 0; c < BOARD_SIZE; c++) {
            ...
            if (!p || p.type !== 'archer' || p.side !== enemy || !onBank(enemy, r))
                continue;
            ...
            for (let dc = -1; dc <= 1; dc++) {
                const bc = c + dc;
                if (bc >= 0 && bc < BOARD_SIZE)
                    _archerSet.add(blockRow * BOARD_SIZE + bc);
            }
        }
    }
    for (let i = 0; i < reserve.length; i++) {
        ...
        for (let r = 0; r < BOARD_SIZE; r++) {
            for (let c = 0; c < BOARD_SIZE; c++) {
                if (state.board[r][c])
                    continue;
                if (type !== 'crossbow' && !canDropOnSide(type, side, r))
                    continue;
                if (_archerSet.has(r * BOARD_SIZE + c))
                    continue;
                tempPiece.type = type;
                tempPiece.side = side;
                ...
                if (legal)
                    out.push({ fromReserve: true, reserveIndex: i, to: { r, c }, type, promotion: false });
            }
        }
    }
    return out;
...
export function isDropLegal(state, side, reserveIndex, to) {
    ...
    if (!entry || state.board[to.r][to.c] || isRiverSquare(to.r))
        return false;
    if (entry.type !== "crossbow" && !canDropOnSide(entry.type, side, to.r))
        return false;
    if (isSquareProtectedByArcher(state, to.r, to.c, opponent(side)))
        return false;
...
export function executeDrop(state, reserveIndex, to) {
    ...
    if (!entry || !isDropLegal(state, state.turn, reserveIndex, to))
        return false;
...
export function afterMoveEvaluation(state) {
    try {
        ...
        const key = boardSignature(state);
        if (state.positionHistory.get(key) >= 3) {
            state.status = "draw";
            state.message = "Draw by position repetition (3 times).";
            return state;
        }
        if ((state.moveCount ?? 0) >= MAX_MOVES) {
            state.status = "draw";
            state.message = "Draw by 400-move limit.";
            return state;
        }
            ...
            for (const s of [SIDE.WHITE, SIDE.BLACK]) {
                if (state.palaceTaken?.[s]) {
                    try {
                        const kings = findKings(state.board);
                        const king = kings[s];
                        if (king && isPalaceSquare(king.r, king.c, s))
                            state.message = `${s === SIDE.WHITE ? "White" : "Black"} captures the palace.`;
                    }
                    catch { }
                }
            }
