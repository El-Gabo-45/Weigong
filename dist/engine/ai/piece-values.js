...
export function seeValue(piece) {
    if (!piece)
        return 0;
    if (piece.promoted)
        return PROMOTED_VALUES[piece.type] ?? (PIECE_VALUES[piece.type] ?? 0) + 120;
...
export function pieceSquareBonus(piece, r, c) {
    ...
    switch (piece.type) {
        case 'pawn':
            ...
            if (piece.promoted)
                bonus += 22;
            if (piece.side === SIDE.WHITE && r >= 7)
                bonus += 14;
            if (piece.side === SIDE.BLACK && r <= 5)
                bonus += 14;
            break;
        case 'horse':
            bonus += (6 - Math.abs(r - 6)) * 4;
            break;
        case 'cannon':
            bonus += prog * 4;
            break;
        ...
        case 'archer':
            ...
            break;
        case 'king':
            bonus += isPalaceSquare(r, c, piece.side) ? 90 : -70;
            break;
        case 'queen':
            bonus += 35;
            break;
        case 'general':
            bonus += 18;
            if (piece.side === SIDE.BLACK && r >= 7)
                bonus -= 200;
            if (piece.side === SIDE.WHITE && r <= 5)
                bonus -= 200;
            break;
        case 'carriage':
            bonus += 10;
