    ...
    for (const side of [SIDE.WHITE, SIDE.BLACK]) {
        ...
        for (let r = 0; r < BOARD_SIZE; r++) {
            for (let c = 0; c < BOARD_SIZE; c++) {
                ...
                if (!p)
                    continue;
                if (p.side === side && p.type === "king" && isPalaceSquare(r, c, side))
                    hasKing = true;
                else if (p.side === enemy && isPalaceSquare(r, c, side))
                    enemyInPalace.push({ r, c, p });
            }
        }
        if (enemyInPalace.length) {
            state.palaceTimers[side].invaded = true;
            state.palaceTimers[side].attackerSide = enemy;
            state.palaceTimers[side].pressure = (state.palaceTimers[side].pressure ?? 0) + 1;
            if (state.palaceTimers[side].pressure >= 3)
                state.palaceTaken[side] = true;
            state.palaceCurse[side].turnsInPalace = (state.palaceCurse[side].turnsInPalace ?? 0) + 1;
