...
import { searchRoot, allocateTime, moveKey, decayHistoryTable } from './search.ts';
...
export function chooseBlackBotMove(state, options = {}) {
    ...
    try {
        const e = evaluate(searchState, rootHash);
        prevScore = e.score;
    }
    catch {
        prevScore = 0;
    ...
    for (let depth = 1; depth <= remainingDepth; depth++) {
        ...
        while (true) {
            ...
            try {
                ...
                if (Math.abs(result.score) > MATE_SCORE - 50)
        ...
        if (depth % 2 === 0 || depth === remainingDepth || depth === 1) {
            dbg.ai(`IDS depth=${depth}`, {
                best: best ? moveKey(best, false) : 'null', score: prevScore,
    ...
    if (best && !best.fromReserve && best.from && best.to) {
        try {
            ...
            if (piece && danceTracker)
                danceTracker.record(state.turn, piece, best.from.r, best.from.c, best.to.r, best.to.c);
        }
        catch { }
    }
...
export async function loadAdaptiveMemory() {
    try {
        ...
        if (!data || data._version !== 4) {
            console.log(`[Memory] Version mismatch — starting fresh`);
            return false;
        }
...
if (typeof window !== 'undefined') {
    window.addEventListener('beforeunload', () => {
        if (typeof navigator !== 'undefined' && navigator.sendBeacon) {
            try {
                navigator.sendBeacon(MEMORY_URL, new Blob([JSON.stringify(adaptiveMemory.toJSON())], { type: 'application/json' }));
            }
            catch { }
        }
    });
