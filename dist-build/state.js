...
export function cloneStateForBot(state) {
    ...
    for (let r = 0; r < BOARD_SIZE; r++)
        board[r] = state.board[r].map(p => p ? { ...p } : null);
    return {
        board,
        turn: state.turn,
        selected: null,
...
export function cancelBotTimer() { if (V.botTimeout) {
    clearTimeout(V.botTimeout);
    V.botTimeout = null;
} V.botThinking = false; V.botToken += 1; }
...
export { boardEl, turnLabel, phaseLabel, reserveWhite, reserveBlack, resetBtn, botToggleBtn, analysisModeBtn, analysisPanel, analysisInfo, analysisBarFill, analysisBarFiller, analysisBarLabel, promotionModal, promotionTitle, promotionText, promotionChoices, ambushModal, ambushTitle, ambushText, ambushChoices, difficultySelect, aiVsAiBtn, trainBtn, messageBar, rulesSummary, moveTimeline, loadGameBtn, loadGameInput, COLS, } from '../src/ui/dom-references.js';
