...
NeuralNetwork::NeuralNetwork(cl_device_id dev, cl_context ctx, cl_command_queue q)
    : device(dev), context(ctx), queue(q), program(nullptr),
      ...
      loss_type(LossType::MSE), optimizer_type(OptimizerType::ADAM),
      learning_rate(0.001f), beta1(0.9f), beta2(0.999f), eps_adam(1e-8f),
      l2_lambda(0.0f), built(false), global_step(0), seed_state(12345),
...
NeuralNetwork::~NeuralNetwork() {
    ...
    safe_release_kernel(k_backprop_delta); safe_release_kernel(k_grad_weight); safe_release_kernel(k_grad_bias);
...
// Reallocates if batch_size > current_batch_size
void NeuralNetwork::ensure_temp_buffers(int batch_size) {
    if (batch_size <= current_batch_size) return; // already big enough
    ...
    for (auto &l : layers) {
        ...
        // Grad buffers never change size, only created once
...
float NeuralNetwork::train_batch(const float *X, const float *Y, int batch_size) {
    ...
    // Calcular pérdida en GPU: loss_fn(pred, target)
    cl_kernel k_loss;
    ...
    for (int i = (int)layers.size() - 1; i >= 0; i--) {
        ...
        if (optimizer_type == OptimizerType::ADAM) {
            ...
            CL_CALL(clSetKernelArg(k_adam_update, 5, sizeof(float), &learning_rate));
            ...
            CL_CALL(clSetKernelArg(k_sgd_update, 3, sizeof(float), &learning_rate));
            size_t g1d_w = (size_t)((size_w + 63) / 64 * 64);
