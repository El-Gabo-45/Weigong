...
export const PROMOTED_VALUES: Partial<Record<PieceType, number>> = {
...
export function pieceSquareBonus(piece: Piece, r: number, c: number): number {
  ...
  switch (piece.type) {
    ...
    case 'general':
      bonus += 18;
      if (piece.side === SIDE.BLACK && r >= 7) bonus -= 200;
      if (piece.side === SIDE.WHITE && r <= 5) bonus -= 200;
      break;
