...
import { SIDE, opponent } from '../constants.js';
import { applyMove } from '../rules/index.js';
import { xorPiece, xorReserves, ZobristTurn, ZobristPalaceWhite, ZobristPalaceBlack } from './hashing.js';
...
function acquireUndo() {
    ...
    u.reservesW = null;
    u.reservesB = null;
    u.reserveRemoved = null;
    u.reserveCaptureAdded = false;
    u.palaceTaken = null;
    u.palaceTimers = null;
    u.palaceCurse = null;
    u.history = null;
    u.historyLength = 0;
    u.positionHistory = null;
    u.lastRepeatedMoveKey = null;
    u.repeatMoveCount = 0;
    u.archerAmbush = null;
    u.selected = null;
    u.legalMoves = null;
    u.promotionRequest = null;
    u.message = null;
    u.status = null;
    u.hash = null;
    u.eval = null;
...
export function makeMove(state, move, promote, currentHash, currentEval = null, precomputedMaps = null) {
        ...
        if (!move.fromReserve && from) {
            ...
            if (piece) {
                ...
                if (target && target.side !== piece.side)
                    evalDiff -= pieceValue(target) + pieceSquareBonus(target, to.r, to.c);
    ...
    if (!move.fromReserve && from && state.board[from.r]?.[from.c])
        newHash = xorPiece(newHash, from.r, from.c, state.board[from.r][from.c]);
    if (to && state.board[to.r]?.[to.c])
        newHash = xorPiece(newHash, to.r, to.c, state.board[to.r][to.c]);
...
export function unmakeMove(state, { undo }) {
    ...
    if (undo.history === null)
        ...
        if (typeof undo.historyLength === 'number')
    ...
    if (undo.palaceCurse)
        state.palaceCurse = undo.palaceCurse;
}
...
export function isSEEPositive(state, move, buildAttackMapFn) {
    if (!move || move.fromReserve || !move.to)
        return true;
    const target = state.board[move.to.r]?.[move.to.c];
    if (!target)
        return true;
    const piece = state.board[move.from.r]?.[move.from.c];
    if (piece && (piece.type === 'archer' || piece.type === 'cannon'))
        return true;
    if (state.palaceCurse?.[state.turn]?.active)
        return true;
