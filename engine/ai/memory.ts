...
interface PatternWeights {
  ...
  palaceDefence: number;
  riverControl: number;
}

interface MemoryJSON {
  _version: number;
  moveScores: [string, MoveScoreEntry][];
  featureScores: [string, FeatureScoreEntry][];
  blunderMoves: [string, number][];
  drawPositions: [string, number][];
...
export function extractFeatures(state: GameState, side: Side): string {
  const enemy: Side = side === SIDE.BLACK ? SIDE.WHITE : SIDE.BLACK;
  ...
  if (kings?.[side] && kings?.[enemy]) {
    ...
    ownKingCol = k1.c; enemyKingCol = k2.c;
...
export class AdaptiveMemory {
  moveScores: Map<string, MoveScoreEntry>;
  featureScores: Map<string, FeatureScoreEntry>;
  blunderMoves: Map<string, number>;
  drawPositions: Map<string, number>;
  maxDrawCount: number;
  patternWeights: PatternWeights;
  gamesPlayed: number;
  gamesWon: number;
  ...
  constructor() {
    this.moveScores    = new Map<string, MoveScoreEntry>();
    this.featureScores = new Map<string, FeatureScoreEntry>();
    this.blunderMoves  = new Map<string, number>();
    this.drawPositions = new Map<string, number>();
    this.maxDrawCount  = 5;
    this.patternWeights = {
  ...
  recordDrawGame(moves: MoveRecord[], drawStatus: string = 'draw'): void {
    ...
    for (const hash of seenHashes) {
      ...
      if (count <= this.maxDrawCount) this.drawPositions.set(hash, count);
    }
  ...
  getDrawPenalty(hash: string): number {
    if (!hash) return 0;
  ...
  recordGame(result: string, moves: MoveRecord[]): void {
    this.gamesPlayed++;
    ...
    for (const m of moves) {
      ...
      if (mk !== null && evalBefore !== null && evalAfter !== null) {
        ...
        const prev: MoveScoreEntry = this.moveScores.get(mk) ?? { total: 0, count: 0 };
        prev.total += delta;
        prev.count++;
      ...
      if (fk !== null) {
        ...
        const prev: FeatureScoreEntry = this.featureScores.get(fk) ?? { total: 0, count: 0 };
        prev.total += delta;
        prev.count++;
  ...
  getMovepenalty(mk: string): number {
    if (!mk) return 0;
  ...
  getFeatureScore(featureKey: string, phaseFactor: number): number {
  ...
  applyWeights(metrics: EvalMetrics): number {
    return (
  ...
  private _adjustWeights(result: string, moves: MoveRecord[]): void {
    ...
    if (n === 0) return;
    const clamp = (v: number) => Math.min(2, Math.max(0.3, v));
    const adj   = (avg: number) => Math.abs(avg - 0.5) > 0.08;
  ...
  getStats(): { gamesPlayed: number; winRate: string; moveMemory: number; featureMemory: number; blunders: number; drawMemory: number; weights: PatternWeights } {
    return {
