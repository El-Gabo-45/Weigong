...
const nnCache = new Map<string, number>();
let nnPredictFn: ((enc: Float32Array) => Promise<number | null>) | null = null;
...
export function setNNPredictFn(fn: (enc: Float32Array) => Promise<number | null>): void {
  nnPredictFn = fn;
}

export function clearNNCache(): void {
  nnCache.clear();
}

export function encodeBoardForNN(board: Board): Float32Array {
...
export function clampNNScore(nnScore: number): number | null {
  if (typeof nnScore !== 'number' || Number.isNaN(nnScore) || !Number.isFinite(nnScore)) return null;
  return Math.max(-1, Math.min(1, nnScore));
}

export function blendScoreWithNN(classicalScore: number, nnScore: number, weight = 0.15, nnCpScale = 300): number {
...
async function getNNScore(board: Board, side: Side): Promise<number | null> {
  ...
  if (nnCache.has(fprint)) return nnCache.get(fprint) ?? null;
...
function safeRatio(a: number, b: number, neutral = 0.5): number {
  const total = a + b;
  return total === 0 ? neutral : a / total;
}

export function gamePhaseFactor(board: Board): number {
  let mat = 0;
  for (const row of board) for (const p of row) if (p && p.type !== 'pawn' && p.type !== 'king') mat += pieceValue(p);
  return Math.min(1.0, mat / 8000);
}

function kingShieldBonus(board: Board, kingR: number, kingC: number, side: Side): number {
  let shield = 0;
...
function countPalaceInvaders(board: Board, side: Side): number {
...
function countOurInvadersInEnemyPalace(board: Board, side: Side): number {
  const enemy = opponent(side);
...
function palaceDefenseScore(ownAttackMap: AttackMapWrapper, enemyAttackMap: AttackMapWrapper, side: Side): number {
  const rows = side === SIDE.BLACK ? PALACE_ROWS_BLACK : PALACE_ROWS_WHITE;
...
function riverAndCrossedScore(board: Board, side: Side, ownAttackMap: AttackMapWrapper): number {
...
function reserveValue(state: GameState, side: Side): number {
...
function palaceDangerAdjustment(state: GameState, side: Side): number {
...
function recklessInvasionPenalty(state: GameState, side: Side): number {
...
function checkmateUrgencyScore(board: Board, side: Side, enemyKingPos: { r: number; c: number } | null, ownMap: AttackMaps, enemyMap: AttackMaps, phaseFactor: number): number {
...
function pawnStructureScore(board: Board, side: Side): number {
  const pawnPositions: { r: number; c: number }[] = [];
  for (let r = 0; r < 13; r++) {
    for (let c = 0; c < 13; c++) {
      const p = board[r][c];
      if (p && p.side === side && p.type === 'pawn') pawnPositions.push({ r, c });
    }
  }
  ...
  for (const pos of pawnPositions) {
    ...
    for (const other of pawnPositions) {
      ...
      if (Math.abs(other.r - pos.r) === 1 && Math.abs(other.c - pos.c) === 1) { isChain = true; break; }
    }
...
function jumpingPieceOverusePenalty(board: Board, side: Side, phaseFactor: number): number {
...
function countInminentPalaceInvasion(state: GameState, side: Side, ownMap: AttackMaps, enemyMap: AttackMaps): number {
...
function developmentScore(board: Board, side: Side, phaseFactor: number): number {
  const openingUrgency = 1 - phaseFactor;
  const devWeight = Math.pow(openingUrgency, DEV_URGENCE_SCALE * 2 + 0.4);
  if (devWeight < 0.05) return 0;
  const isBlack = side === SIDE.BLACK;
  const homeBackRank = isBlack ? 0 : 12;
  ...
  for (let r = 0; r < 13; r++) {
    for (let c = 0; c < 13; c++) {
      ...
      if (p.type === 'pawn') {
        ...
        if (r === homeBackRank) undeveloped++;
        else { movedPieces++; piecesActive++; if (isBlack ? (r >= 7) : (r <= 5)) piecesInEnemyTerritory++; }
      }
    }
  }
...
function kingSafetyFast(state: GameState, side: Side, ownByPiece: { _arr: Uint8Array }, enemyAttackMap: { _arr: Uint8Array }, phaseFactor: number, kingPos: { r: number; c: number } | null): number {
...
function repetitionPenalty(hash: bigint, history: bigint[]): number {
  let seen = 0;
  for (const h of history) if (h === hash) seen++;
  return Math.max(0, seen - 1) * REPEAT_PENALTY;
}

function kingRecentMovePenalty(state: GameState): number {
...
export function evaluate(state: GameState, hash: bigint, precomputedMaps: { black: AttackMaps; white: AttackMaps } | null = null, skipMemory = false): EvalResult {
  ...
  if (materialAdv > ADV_ADVANTAGE_THRESHOLD) {
    for (let r = 0; r < 13; r++) {
      for (let c = 0; c < 13; c++) {
        ...
        if (p.type === 'king') continue;
        if (p.type !== 'pawn') { if (r <= 5) blackAdvUrgency += ADV_CROSSED_RIVER_BONUS; }
        else { blackAdvUrgency += r * ADV_PAWN_PUSH_BONUS / 3; }
      }
    }
  ...
  if (materialAdv < -ADV_ADVANTAGE_THRESHOLD) {
    for (let r = 0; r < 13; r++) {
      for (let c = 0; c < 13; c++) {
        ...
        if (p.type === 'king') continue;
        if (p.type !== 'pawn') { if (r >= 7) whiteAdvUrgency += ADV_CROSSED_RIVER_BONUS; }
        else { const advance = 12 - r; whiteAdvUrgency += advance * ADV_PAWN_PUSH_BONUS / 3; }
      }
    }
