...
const killerMoves = new Map<number, number[]>();
const historyTable: { white: Map<number, number>; black: Map<number, number> } = { white: new Map(), black: new Map() };
...
export class GameDanceTracker {
  windowSize: number;
  visits: Record<string, Map<string, Map<number, number>>>;
  centroids: Record<string, Map<string, { sumRow: number; count: number }>>;
  ring: Record<string, Array<{ key: string; cellIdx: number; toRow: number }>>;
  ...
  record(side: string, piece: any, fr: number, fc: number, tr: number, tc: number): void {
    ...
    ce.sumRow += tr; ce.count++;
    if (ring.length >= this.windowSize) {
      const evicted = ring.shift()!;
      const epm = sv.get(evicted.key);
      if (epm) {
        const prev = epm.get(evicted.cellIdx) ?? 0;
        if (prev <= 1) epm.delete(evicted.cellIdx); else epm.set(evicted.cellIdx, prev - 1);
        if (epm.size === 0) sv.delete(evicted.key);
      }
      const ece = sc.get(evicted.key);
      if (ece) { ece.sumRow -= evicted.toRow; ece.count = Math.max(0, ece.count - 1); if (ece.count === 0) sc.delete(evicted.key); }
    }
    ring.push({ key, cellIdx, toRow: tr });
  }

  oscillation(side: string, piece: any, fr: number, fc: number, tr: number, tc: number): number {
  ...
  stagnation(side: string, piece: any, fr: number, fc: number, tr: number, tc: number): number {
    if (!piece || piece.type === 'king') return 0;
    const id = piece.id ?? `${piece.type}@${fr},${fc}`;
    const key = `${piece.type}@${id}`;
...
let _activeDanceTracker: GameDanceTracker | null = null;
...
export function moveKey(move: any, promote = false): string {
  if (!move) return 'null';
...
export function moveKeyUint32(move: any, promote = false): number {
  if (!move) return 0;
  if (move.fromReserve) {
...
function storeKiller(depth: number, move: any): void {
  const key = moveKeyUint32(move, move.promotion ?? false);
  const prev = killerMoves.get(depth) ?? [];
  if (prev[0] === key) return;
  killerMoves.set(depth, [key, prev[0] ?? null].filter(Boolean).slice(0, KILLER_SLOTS) as number[]);
}

function killerScore(depth: number, mk: number): number {
  const arr = killerMoves.get(depth);
  if (!arr) return 0;
  return arr[0] === mk ? 900 : arr[1] === mk ? 650 : 0;
}

function historyScore(side: string, mk: number): number { return historyTable[side].get(mk) ?? 0; }

function storeHistory(side: string, move: any, depth: number): void {
  const key = moveKeyUint32(move, move.promotion ?? false);
  historyTable[side].set(key, (historyTable[side].get(key) ?? 0) + depth * depth);
}

export function decayHistoryTable(): void {
...
function isQuiet(state: GameState, move: any, promote: boolean): boolean {
  return !move || move.fromReserve || promote ? false : !state.board?.[move.to?.r]?.[move.to?.c];
}

function terminalScore(state: GameState, depth: number, precomputedMoves: any[] | null = null): number | null {
  try {
    const legal = precomputedMoves ?? getAllLegalMoves(state, state.turn as any) ?? [];
    const inCheck = isKingInCheck(state, state.turn as any);
    if (legal.length === 0) return inCheck ? (state.turn === 'black' ? -MATE_SCORE + depth : MATE_SCORE - depth) : 0;
    return null;
  } catch { return null; }
}

function isTactical(state: GameState, move: any): boolean {
  ...
  if (target) { if (isPalaceSquare(move.to.r, move.to.c, state.turn)) return true; return true; }
  return !moving.promoted && isPromotionAvailableForMove(state, move.from, move.to);
}

function givesCheck(state: GameState, move: any): boolean {
  ...
  const inCheck = isKingInCheck(state, opponent(state.turn) as any);
  unmakeMove(state, md as any);
  return inCheck;
}

function countRepetitions(history: bigint[], hash: bigint): number {
  let seen = 0;
  for (const h of history) if (h === hash) seen++;
  return seen;
}

function _extractMoveInfo(md: any, move: any): { movedPiece: any; capturedPiece: any } {
  ...
  if (md.undo && md.undo.cells) {
    for (let i = 0; i < md.undo.cellCount; i++) {
      ...
      if (!move.fromReserve && move.from && cell.r === move.from.r && cell.c === move.from.c) movedPiece = cell.p;
      else if (move.to && cell.r === move.to.r && cell.c === move.to.c) capturedPiece = cell.p;
    }
  }
  return { movedPiece, capturedPiece };
}

function _applyMaps(maps: AttackMapsPair | null, state: GameState, move: any, md: any, promote: boolean): AttackMapsPair | null {
  if (!maps) return null;
  try {
...
function _rebuildMaps(maps: AttackMapsPair | null, board: GameState['board']): void {
  if (!maps) return;
  try {
    (maps._blackInc as any)?.rebuild(board);
    (maps._whiteInc as any)?.rebuild(board);
    if ((maps as any)._blackInc) (maps as any).black = (maps as any)._blackInc.get();
    if ((maps as any)._whiteInc) (maps as any).white = (maps as any)._whiteInc.get();
  } catch {}
}

function quiescence(state: GameState, alpha: number, beta: number, deadline: number, hash: bigint, staticEval: number | null = null, qdepth = QSEARCH_MAX_DEPTH, maps: AttackMapsPair | null = null): number {
  if (now() > deadline) throw new SearchTimeout();
  const maximizing = state.turn === 'black';
  const inCheck = isKingInCheck(state, state.turn as any);
  const precomputed = maps ? { black: (maps as any).black, white: (maps as any).white } : null;
  ...
  if (!inCheck) {
    if (maximizing) { if (best >= beta) return best; alpha = Math.max(alpha, best); if (ev + QDELTA_MARGIN < alpha) return ev; }
    else { if (best <= alpha) return best; beta = Math.min(beta, best); if (ev - QDELTA_MARGIN > beta) return ev; }
  }
  const moves = getAllLegalMoves(state, state.turn as any)
    .filter((m: any) => m && (inCheck || isTactical(state, m)))
    .sort((a: any, b: any) => moveOrderScore(state, b, 0) - moveOrderScore(state, a, 0));
  for (const move of moves) {
    ...
    for (const promote of getBranches(state, move)) {
      ...
      } finally { unmakeMove(state, md as any); _rebuildMaps(maps, state.board); }
    }
  }
  return best;
}

export function search(state: GameState, depth: number, alpha: number, beta: number, deadline: number, tt: any, hash: bigint, staticEval: number | null = null, isNullMove = false, maps: AttackMapsPair | null = null): number {
  if (now() > deadline) throw new SearchTimeout();
  if (state.history?.length >= 2) {
    const reps = countRepetitions(state.history, hash);
    if (reps >= 2) { return state.turn === 'black' ? -DRAW_CONTEMPT : DRAW_CONTEMPT; }
    if (reps === 1) {
      const precomputed = maps ? { black: (maps as any).black, white: (maps as any).white } : null;
  ...
  if (cached && cached.depth >= depth) {
    ...
    if (cached.flag === 1 && cached.score <= alpha) return alpha;
    if (cached.flag === 2 && cached.score >= beta) return beta;
  }
  const rawMoves = getAllLegalMoves(state, state.turn as any);
  ...
  if (!inCheck && depth <= 2) {
    ...
    if (maximizing && staticEval + razorMargin <= alpha) return quiescence(state, alpha, beta, deadline, hash, staticEval, QSEARCH_MAX_DEPTH, maps);
    if (!maximizing && staticEval - razorMargin >= beta) return quiescence(state, alpha, beta, deadline, hash, staticEval, QSEARCH_MAX_DEPTH, maps);
  }
    ...
    for (let r = 0; r < 13 && !kingAttacked; r++)
      for (let c = 0; c < 13 && !kingAttacked; c++) {
        ...
        if (p && p.side === state.turn && p.type === 'king')
          kingAttacked = isSquareAttacked(state.board, r, c, opponent(state.turn) as any, state as any);
      }
    if (!kingAttacked) {
      const saved = state.turn;
      state.turn = opponent(saved) as any;
      const R = depth > 6 ? 3 : 2;
  ...
  if (!ttMoveKey && depth >= 5 && !inCheck) effectiveDepth = depth - 1;
  let hashFlag = 1, bestMoveForTT: any = null;
  ...
  if (effectiveDepth >= 3 && !inCheck && Math.abs(beta) < MATE_SCORE / 2) {
    ...
    for (const move of moves) {
      ...
      try {
        ...
        if (probScore >= beta) { return probScore; }
      } finally { unmakeMove(state, md as any); _rebuildMaps(maps, state.board); }
    }
  }
  ...
  for (const move of moves) {
    ...
    for (const promote of getBranches(state, move)) {
      ...
      try {
        ...
        if (maximizing) { if (score > best) { best = score; bestMoveForTT = move; } if (best > alpha) { alpha = best; hashFlag = 0; } }
        else { if (score < best) { best = score; bestMoveForTT = move; } if (best < beta) { beta = best; hashFlag = 0; } }
        if (alpha >= beta) {
...
export function searchRoot(state: GameState, depth: number, alpha: number, beta: number, deadline: number, tt: any, hash: bigint, prevScore: number, rootNNByMoveKey: any = null, danceTracker: any = null): { bestMove: NormalizedMove | null; score: number } {
  const maximizing = state.turn === 'black';
  const cached = tt.get(hash);
  const ttMoveKey = cached?.bestMoveKey ?? null;
  ...
  scoredMoves.sort((a, b) => b.score - a.score);
  const moves = scoredMoves.map(sm => sm.move);
  if (!moves.length) { const term = terminalScore(state, depth, rawMoves); return { bestMove: null, score: term ?? prevScore }; }
  let rootMaps: any = null;
  try { rootMaps = createIncrementalMaps(state.board); } catch { rootMaps = null; }
  const thirdRepMoveKeys = new Set<number>();
  if (state.history?.length >= 2 && moves.length > 1) {
    for (const m of moves) {
      for (const pr of getBranches(state, m)) {
        ...
        if (probe.action && probe.undo) { if (countRepetitions(state.history, probe.hash) >= 3) thirdRepMoveKeys.add(moveKeyUint32(m, pr)); }
        if (probe.undo) unmakeMove(state, probe as any);
      }
    }
    if (thirdRepMoveKeys.size >= moves.length) thirdRepMoveKeys.clear();
  }
  let bestMove: NormalizedMove | null = null, bestScore = maximizing ? -INF : INF, moveCount = 0;
  const branchesCache = new Map<any, boolean[]>();
  for (const move of moves) {
    if (now() > deadline) throw new SearchTimeout();
    let branches = branchesCache.get(move);
    if (!branches) { branches = getBranches(state, move); branchesCache.set(move, branches); }
    for (const promote of branches) {
      if (thirdRepMoveKeys.has(moveKeyUint32(move, promote))) continue;
      moveCount++;
      ...
      try {
        ...
        if (moveCount === 1) { score = -search(state, depth - 1, -beta, -alpha, deadline, tt, md.hash, childEval, false, childMaps ? rootMaps : null); }
        else {
          score = -search(state, depth - 1, -alpha - 1, -alpha, deadline, tt, md.hash, childEval, false, childMaps ? rootMaps : null);
          if (score > alpha && score < beta) score = -search(state, depth - 1, -beta, -alpha, deadline, tt, md.hash, childEval, false, childMaps ? rootMaps : null);
        }
      ...
      } finally { unmakeMove(state, md as any); _rebuildMaps(rootMaps, state.board); }
    }
  }
    ...
    for (const move of moves) {
      ...
      for (const promote of getBranches(state, move)) {
        ...
        try {
          ...
          if (alpha >= beta) return { bestMove, score: bestScore };
        } finally { unmakeMove(state, md as any); _rebuildMaps(rootMaps, state.board); }
      }
    }
  }
...
function getBranches(state: GameState, move: any): boolean[] {
  ...
  const destAttacked = isSquareAttacked(state.board, move.to.r, move.to.c, enemySide as any, state as any);
  return destAttacked ? [true, false] : [true];
}

function isValidMove(move: any): boolean {
  if (!move || typeof move !== 'object') return false;
  if (move.fromReserve) return Number.isInteger(move.reserveIndex) && move.to && Number.isInteger(move.to.r) && Number.isInteger(move.to.c);
...
function isBacktrack(state: GameState, move: any): boolean {
...
function kingPenalty(state: GameState, move: any): number {
  ...
  if (!isKingInCheck(state, piece.side as any)) pen += 40;
  return pen;
}

function kingShufflePen(state: GameState, move: any): number {
...
function boardChecksum(board: GameState['board']): number {
  let cs = 0;
  for (let r = 0; r < 13; r++) { const row = board[r]; for (let c = 0; c < 13; c++) { const p = row[c]; if (!p) continue; const id = (p.side === 'black' ? 0x8000 : 0) | (r << 8) | (c << 4) | (p.type.charCodeAt(0) & 0xF); cs ^= id ^ (p.promoted ? 0x10000 : 0); } }
  return cs;
}

function countMaterial(board: GameState['board']): number {
  const cached = getMaterialCache();
  if (cached >= 0) return cached;
  let count = 0;
  for (let r = 0; r < 13; r++) { const row = board[r]; for (let c = 0; c < 13; c++) { const p = row[c]; if (p && p.type !== 'king' && p.type !== 'pawn') count++; } }
  setMaterialCache(count);
  return count;
}

function moveOrderScore(state: GameState, move: any, depth: number, currentHash: bigint | null = null): number {
  ...
  if (!move.fromReserve && move.from && move.to && isPromotionAvailableForMove(state, move.from, move.to) && !moving.promoted) score += 600;
  ...
  if (currentHash !== null && state.history?.length >= 2) {
    ...
    if (seen >= 2) score -= 20000; else if (seen === 1) score -= 4000;
  }
  const enemy = opponent(side);
  if (isPalaceSquare(move.to.r, move.to.c, enemy)) score += PALACE_PRESSURE_BONUS;
  if (target && target.side === enemy && isPalaceSquare(move.to.r, move.to.c, side)) score += 3000;
  if (state.palaceCurse?.[enemy]?.active && isPalaceSquare(move.to.r, move.to.c, enemy)) score += 2000;
  return score;
}

export function allocateTime(startTime: number, timeLimitMs: number, moveCount = 30): number {
