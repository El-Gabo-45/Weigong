// ═════════════════════════════════════════════════════
//  Move Generation (EN/ES)
// ES: Move Generation (EN/ES)
// ═════════════════════════════════════════════════════
...
import type { Board, Piece, GameState, MoveCandidate, Side } from '../types.js';

// ── OPT-ARCHER: Pre-computed archer-protection set ───────────────────────────
const _archerProtected = new Set<number>();

function _buildArcherProtected(board: Board, side: Side): void {
...
export interface AddIfValidOpts {
  canCaptureOwn?: boolean;
  special?: string | null;
}

export function addIfValid(moves: MoveCandidate[], board: Board, piece: Piece, fromR: number, fromC: number, toR: number, toC: number, opts: AddIfValidOpts = {}): void {
...
export function rayMoves(board: Board, piece: Piece, r: number, c: number, dirs: [number, number][], maxSteps = 99, opts: { noCrossRiver?: boolean } = {}): MoveCandidate[] {
...
export function jumpMoves(board: Board, piece: Piece, r: number, c: number, deltas: [number, number][], opts: { noCrossRiver?: boolean } = {}): MoveCandidate[] {
  const moves: MoveCandidate[] = [];
...
export function pseudoMovesForPiece(board: Board, piece: Piece, r: number, c: number, state?: GameState): MoveCandidate[] {
  ...
  if (!piece.promoted) {
    ...
    if (kind === "horse") {
      const isPathClear = (path: [number, number][]) => { for (const [pr, pc] of path) { if (!inBounds(pr, pc) || board[pr][pc]) return false; } return true; };
      const horseMoves = [
        { dest: [r+2, c+1], pathA: [[r+1, c], [r+2, c]] as [number, number][], pathB: [[r, c+1], [r+1, c+1]] as [number, number][] },
        { dest: [r+2, c-1], pathA: [[r+1, c], [r+2, c]] as [number, number][], pathB: [[r, c-1], [r+1, c-1]] as [number, number][] },
        { dest: [r-2, c+1], pathA: [[r-1, c], [r-2, c]] as [number, number][], pathB: [[r, c+1], [r-1, c+1]] as [number, number][] },
        { dest: [r-2, c-1], pathA: [[r-1, c], [r-2, c]] as [number, number][], pathB: [[r, c-1], [r-1, c-1]] as [number, number][] },
        { dest: [r+1, c+2], pathA: [[r, c+1], [r, c+2]] as [number, number][], pathB: [[r+1, c], [r+1, c+1]] as [number, number][] },
        { dest: [r+1, c-2], pathA: [[r, c-1], [r, c-2]] as [number, number][], pathB: [[r+1, c], [r+1, c-1]] as [number, number][] },
        { dest: [r-1, c+2], pathA: [[r, c+1], [r, c+2]] as [number, number][], pathB: [[r-1, c], [r-1, c+1]] as [number, number][] },
        { dest: [r-1, c-2], pathA: [[r, c-1], [r, c-2]] as [number, number][], pathB: [[r-1, c], [r-1, c-1]] as [number, number][] },
      ];
    ...
    if (kind === "pawn") {
      const skip = (row: number) => isRiverSquare(row) ? row + f : row;
      let nr = skip(r + f);
    ...
    if (kind === "crossbow") {
      const pushWithRiverRule = (dr: number, dc: number) => {
        const step1r = r + dr;
        const step1c = c + dc;
...
export function getLegalMovesForSquare(state: GameState, r: number, c: number, opts: { skipKingCheck?: boolean } = {}): MoveCandidate[] {
