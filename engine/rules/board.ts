...
import type { Side, Piece, Board, BoardRow, GameState } from '../types.js';

let _pieceIdCounter = 0;

// Factory: create a piece with unique ID
// ES: Crea una pieza con ID único
export function makePiece(type: string, side: Side, promoted = false): Piece {
  const id = `p_${Date.now()}_${++_pieceIdCounter}_${Math.random().toString(36).slice(2, 6)}`;
  return { id, type: type as any, side, promoted, locked: false } as Piece;
}
...
export function initialLayout(): Board {
  const board: Board = Array.from({ length: BOARD_SIZE }, () => Array(BOARD_SIZE).fill(null));

  const topBack: (string | null)[] = ["tower", "cannon", "horse", "priest", "elephant", "general", "king", "queen", "elephant", "priest", "horse", "cannon", "tower"];
  const topMid: (string | null)[] = [null, "carriage", null, null, null, null, "archer", null, null, null, null, "carriage", null];
...
export function insideOwnPalaceRow(side: Side, row: number): boolean {
  return side === SIDE.BLACK ? row >= 0 && row <= 2 : row >= 10 && row <= 12;
}

export function sameSide(piece: Piece | null, target: Piece | null): boolean {
...
export function lineClear(board: Board, r1: number, c1: number, r2: number, c2: number): boolean {
...
export function countBetween(board: Board, r1: number, c1: number, r2: number, c2: number): number {
  const dr = Math.sign(r2 - r1);
  const dc = Math.sign(c2 - c1);
  let r = r1 + dr;
  let c = c1 + dc;
...
export function pathSquares(r1: number, c1: number, r2: number, c2: number): [number, number][] {
...
export function pathIsClear(board: Board, r1: number, c1: number, r2: number, c2: number): boolean {
  const dr = Math.sign(r2 - r1);
  const dc = Math.sign(c2 - c1);
  let r = r1 + dr;
  let c = c1 + dc;
...
export interface KingPositions {
  [side: string]: { r: number; c: number; piece: Piece };
}
...
export function findKings(board: Board): KingPositions {
  const out: KingPositions = {};
  let found = 0;
...
export function isEnemy(pieceA: Piece | null, pieceB: Piece | null): boolean {
  return pieceA !== null && pieceB !== null && pieceA.side !== pieceB.side;
}

// ─── boardSignature optimizations ────────────────────────────────────────────
const _T: Record<string, { w: string[]; b: string[] }> = Object.create(null);
...
const _sigCells = new Array<string>(BOARD_SIZE * BOARD_SIZE);
const _RES_TYPES = ['tower', 'general', 'pawn', 'crossbow'];

function _reserveKey(reserve: Piece[]): string {
...
export function cloneState(state: GameState): GameState {
  ...
  for (let r = 0; r < BOARD_SIZE; r++) {
    ...
    const dst: BoardRow = new Array(BOARD_SIZE);
    for (let c = 0; c < BOARD_SIZE; c++) {
      const p = src[c];
      dst[c] = p
        ? { id: p.id, type: p.type, side: p.side, promoted: p.promoted, locked: (p as any).locked }
        : null;
    }
    board[r] = dst;
  }
  ...
  return {
    ...
    legalMoves: [],
    reserves: {
      white: state.reserves.white.map(p => ({ type: p.type, side: p.side, promoted: p.promoted ?? false, id: p.id, locked: (p as any).locked ?? false })),
      black: state.reserves.black.map(p => ({ type: p.type, side: p.side, promoted: p.promoted ?? false, id: p.id, locked: (p as any).locked ?? false })),
    },
    ...
    lastMove: state.lastMove ? { ...state.lastMove } as any : null,
