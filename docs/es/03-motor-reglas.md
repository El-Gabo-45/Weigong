...
El motor de reglas es el núcleo lógico del juego. Se encarga de la generación de movimientos, validación de legalidad, detección de jaque/mate y todas las mecánicas especiales del juego.
...
| `core.js` | Motor central: `applyMove`, `getAllLegalMoves`, colocaciones, evaluación post-movimiento |
...
| `check.js` | Detección de jaque, casillas atacadas, generación de casillas de ataque |
...
| `game.js` | Fábrica de estado del juego: `createGame()`, `resetGame()` |
...
```javascript
{
  ...
  message: "Juego listo.",      // Mensaje de estado para la UI
  ...
  palaceCurse: {                // Maldición de palacio
...
pseudoMovesForPiece()      ← Genera todos los movimientos posibles sin verificar legalidad
       │
       ▼
getLegalMovesForSquare()   ← Filtra movimientos pseudo-legales:
       │                      1. Restricciones de tipo (arquero/carruaje en su lado)
       │                      2. No puede terminar en el río
       │                      3. No puede capturar piezas propias
       │                      4. No puede moverse a casillas protegidas por arquero enemigo
       │                      5. Verificación de jaque propio (simular + verificar)
       ▼
...
Genera movimientos pseudo-legales basados en el tipo de pieza. Cada movimiento retorna:
...
1. Obtiene movimientos pseudo-legales de `pseudoMovesForPiece`.
2. Filtra restricciones de movimiento:
   - Arqueros y carruajes solo pueden moverse en su propio lado.
   - Ninguna pieza puede terminar en la fila del río.
   - No puede moverse a casillas protegidas por arquero enemigo.
3. Simula cada movimiento y verifica que el rey propio no quede en jaque.
4. Retorna solo movimientos legales.

### Utilidades de Movimiento

| Función | Descripción |
|---------|-------------|
| `rayMoves(board, piece, r, c, dirs, maxSteps)` | Genera movimientos en línea recta (torre, reina, sacerdote). Se detiene al encontrar una pieza. |
| `jumpMoves(board, piece, r, c, deltas)` | Genera saltos a posiciones fijas (tipo caballo). |
| `addIfValid(moves, board, piece, fromR, fromC, toR, toC)` | Agrega movimiento si está dentro del tablero y no captura pieza propia. |
...
1. **Colocación desde reserva:** Retira la pieza de la reserva y la coloca en el tablero.
2. **Movimientos normales:** Mueve la pieza del origen al destino. Si hay captura:
   - Pieza capturada reutilizable → va a la reserva del captor.
   - Pieza no reutilizable → eliminada permanentemente.
3. **Promoción:** Si la pieza llega a la zona de promoción y el jugador elige promoverla.
...
7. **Historial de posiciones:** Registra la firma de la posición para detección de repetición.
...
2. Agrega colocaciones legales desde la reserva → `getLegalReserveDrops()`.
3. Retorna lista de `{ from: {r,c}, to: {r,c} }` y colocaciones `{ fromReserve: true, reserveIndex, to }`.

### `getLegalReserveDrops(state, side)`

Genera colocaciones legales desde la reserva:

1. Para cada pieza en la reserva, itera sobre todas las casillas vacías.
2. Verifica: casilla vacía, no río, lado correcto, no protegida por arquero enemigo.
3. Simula la colocación y verifica que el rey propio no quede en jaque.
...
3. **Mate de palacio:** Si el rey está atrapado dentro de su propio palacio con enemigos dentro y sin escape ni posibilidad de expulsar a los invasores.
4. **Palacio tomado:** Si un palacio ha estado bajo asedio el tiempo suficiente.
...
1. Localiza al rey en el tablero con `findKings()`.
...
Verifica si una casilla está atacada por alguna pieza del `bySide`:

1. Para cada pieza del bando atacante, genera sus casillas de ataque con `attackSquaresForPiece()`.
2. Si alguna casilla de ataque coincide con `(r, c)` → casilla atacada.
...
Genera las casillas que una pieza ataca (no necesariamente donde puede moverse legalmente). Similar a `pseudoMovesForPiece` pero usado específicamente para detección de amenazas.
...
| `countBetween(board, r1,c1, r2,c2)` | Cuenta piezas entre dos posiciones (para el cañón) |
...
2. **Si hay invasores:**
   ...
   - Si la presión llega a 3 → `palaceTaken[side] = true`.
   - Incrementa `palaceCurse[side].turnsInPalace`.
   - Si los turnos en palacio llegan a 3 → activa maldición (`palaceCurse[side].active = true`).
3. **Si no hay invasores:** Reinicia todos los contadores.

### `isPalaceCursedFor(state, side)`

Retorna `true` si la maldición del palacio está activa para el bando dado. La maldición modifica el movimiento del Rey y la Reina.
...
- Las casillas diagonal frente-izquierda y frente-derecha.

### `isSquareProtectedByArcher(state, targetRow, targetCol, protectorSide)`

Verifica si una casilla está en la zona bloqueada de cualquier arquero del bando protector. Las piezas no pueden moverse a ni colocarse en casillas protegidas por arqueros enemigos.
