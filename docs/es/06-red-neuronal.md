...
El proyecto incluye una red neuronal implementada en C++ con aceleración GPU vía OpenCL, diseñada para evaluar posiciones del tablero con mayor precisión que la evaluación heurística.
...
```
Entrada (4056 neuronas)
    │
    ▼
Capa Densa (512 neuronas) ── LeakyReLU + BatchNorm + Dropout
    │
    ▼
Capa Densa (256 neuronas) ── LeakyReLU + BatchNorm + Dropout
    │
    ▼
Capa Densa (128 neuronas) ── LeakyReLU + BatchNorm + Dropout
    │
    ▼
Capa Densa (64 neuronas) ── LeakyReLU + BatchNorm + Dropout
    │
    ▼
Capa de Salida (1 neurona) ── Tanh
```

### Dimensión de Entrada

```
Entrada = 13 × 13 × 24 = 4056 neuronas
```
...
// Valor = 1.0 si la pieza está presente, 0.0 si no
```

### Salida

Un único valor en el rango [-1, 1] (tanh):
...
| `src/main.cpp` | Punto de entrada (lee JSON desde stdin) |
| `src/nn_kernel.cl` | Kernels OpenCL para GPU |
| `nn_kernel.cl` | Copia del kernel en la raíz del módulo |
...
// El proyecto usa HUBER (robusto ante outliers)
```
...
// El proyecto usa ADAMW (con decaimiento de pesos desacoplado)
```
...
| `k_matmul_transposed` | Multiplicación transpuesta (backward pass) |
...
| `k_bias_add` | Adición de bias |
| `k_backprop_delta` | Cálculo de delta en backpropagation |
...
| `k_gradient_clip` | Recorte de gradientes |

---

## 6.4 Bridge Node.js ↔ C++ (`nn-bridge.js`)
...
3. Lanza el proceso `nn_train` con datos via stdin.
...
Retorna información sobre el modelo actual (arquitectura, tamaño, etc.).

#### `diagnoseGames(games)`

Diagnóstico pre-entrenamiento:
- Cuenta partidas decisivas vs empates.
- Mide cobertura de datos para la NN.
- Reporta distribución de estados finales.

### Construcción de Targets

```javascript
function buildTargets(game) {
  // Normalización: tanh(evalScore / 300)  ← más agresivo que tanh(x/1000)
  //
  // Partida decisiva:
  //   target = (1 - resultWeight) × heurístico + resultWeight × resultSign
  //   resultWeight = 0.1 + 0.7 × progreso  (crece a lo largo de la partida)
  //
  // Empate por límite de movimientos:
  //   Solo primera mitad, heurístico × 0.25
  //
  // Empate real:
  //   Heurístico × 0.5
}
...
El Makefile genera objetos en `build/` y el binario `nn_train` en la raíz del módulo.
